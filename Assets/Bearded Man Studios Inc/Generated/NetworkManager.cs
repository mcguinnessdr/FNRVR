...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] VRPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is VRPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (VRPlayerNetworkObject.Length > 0 && VRPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(VRPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateVRPlayer instead, its shorter and easier to type out ;)")]
		public VRPlayerBehavior InstantiateVRPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(VRPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<VRPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<VRPlayerBehavior>().networkObject = (VRPlayerNetworkObject)obj;
		...
		public VRPlayerBehavior InstantiateVRPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(VRPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<VRPlayerBehavior>();
